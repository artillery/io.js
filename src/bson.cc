...
MaybeLocal<String> BSONDeserializer::ReadCString()
{
  char* start = p;
  while(*p++ && (p < pEnd)) { }
  if(p > pEnd) {
...
Handle<Value> BSONDeserializer::DeserializeDocumentInternal(bool promoteLongs)
{
  ...
  while(HasMoreData())
  {
    ...
    auto name = ReadCString();
    if(name.IsEmpty()) ThrowAllocatedStringException(64, "Bad BSON Document: illegal CString");
    // name->Is
    const Handle<Value>& value = DeserializeValue(type, promoteLongs);
    CHECK(returnObject->CreateDataProperty(context, name.ToLocalChecked(), value).IsJust());
  }
