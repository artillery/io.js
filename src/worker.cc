...
#define EXPORT_MESSAGE_TYPE(type)                                                        \
  do {                                                                                   \
  CHECK(target->DefineOwnProperty(context, FIXED_ONE_BYTE_STRING(env->isolate(), #type), \
...
#undef EXPORT_MESSAGE_TYPE
  ...
  if (env->is_worker_thread()) {
    ...
    t->InstanceTemplate()->SetInternalFieldCount(1);
    Local<Object> worker_wrapper = t->GetFunction()->NewInstance(context).ToLocalChecked();
...
void WorkerContext::Run() {
...

  {
    ...
    if (!JsExecutionAllowed()) {
      ...
      Local<Value> user_data = v8::JSON::Parse(
          worker_isolate(), String::NewFromUtf8(worker_isolate(), user_data_)
          ).ToLocalChecked();
      free(user_data_);
      user_data_ = nullptr;
      CHECK(env->process_object()->DefineOwnProperty(context,
                                               FIXED_ONE_BYTE_STRING(worker_isolate(),
                                                                     "workerData"),
                                               user_data,
                                               v8::ReadOnly).IsJust());
