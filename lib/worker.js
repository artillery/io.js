...
Worker.prototype.postMessage = function(payload, transferList) {
  this[checkAliveSymbol]();
  this[workerContextSymbol].postMessage(payload,
                                        (transferList != null) ? transferList : EMPTY_ARRAY,
...
if (process.isWorkerThread) {
  const postMessage = function(payload, transferList, type) {
    ...
    WorkerContextBinding.workerWrapper._postMessage(payload, transferList, type);
  };
  const workerFatalError = function(er) {
    ...
    if (er instanceof Error) {
      ...
      try {
        ...
        // TODO: with BSON serialization, JSONStringify is not sufficient to guarantee success.
  ...
  Worker.postMessage = function(payload, transferList) {
    postMessage(payload, (transferList != null) ? transferList : EMPTY_ARRAY, WorkerContextBinding.USER);
  };
